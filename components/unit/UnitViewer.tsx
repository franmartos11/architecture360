    ...
    <div className="flex h-screen bg-slate-950 overflow-hidden relative">
      ...
      >
        ...
        <div className="p-5 flex-1">
          ...
          <p className="text-sm text-slate-400 uppercase tracking-wide mb-4">{unit.modelName}</p>

          {/* Price */}
          <button 
            onClick={() => openContact()}
            className="w-full py-2 rounded-lg border border-white/10 text-sm text-slate-200 font-medium hover:bg-white/5 transition-colors mb-4"
          >
            Consultar precio
          </button>

          {/* View tabs (Galería / Planta 3D / Planos) */}
          <div className="flex items-center justify-around border border-white/5 rounded-xl p-1 mb-5 bg-slate-950">
            {TABS.map(tab => (
              <button
                key={tab.id}
            ...
            <div className="flex items-center gap-3">
              {[
                ...
                <button
                  ...
                  className="w-10 flex-shrink-0 h-28 flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/5 transition-colors"
                >
                  {item.icon}
                </button>
              ))}
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-4 pt-4 pointer-events-none">
          ...
          <div className="flex items-center gap-2 pointer-events-auto">
            <span className="text-sm font-medium text-slate-200 bg-white/5 border border-white/10 shadow-lg rounded-lg px-3 py-1.5 backdrop-blur-md">
              Planta {floorNumber}
            </span>
            <button
              onClick={() => router.push(`/proyecto/${projectSlug}/edificio/${buildingId}`)}
              className="px-4 py-1.5 rounded-lg bg-white/5 border border-white/10 shadow-lg text-sm font-medium text-slate-200 hover:bg-white/10 backdrop-blur-md transition-colors"
            >
              Cambiar planta
            </button>
          </div>
        ...
        <div className="absolute right-4 top-20 z-20 flex flex-col gap-2 pointer-events-auto">
          {TABS.map(tab => (
            <button
              ...
              onClick={() => setActiveTab(tab.id)}
              title={tab.label}
              className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-150 shadow-lg backdrop-blur-md border ${
                activeTab === tab.id
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'planta3d' && (
              ...
              >
                <div className="relative w-full h-full">
                  {unit.orientation && (
                    <div className="absolute top-24 left-6 flex items-center justify-center gap-2 bg-slate-900/80 border border-white/10 backdrop-blur-md rounded-full px-4 py-2 text-white shadow-xl z-20">
                      <Compass className="w-5 h-5 text-amber-500" />
                      <span className="font-semibold text-sm tracking-widest">{unit.orientation}</span>
                    </div>
                  )}
                  {unit.floorPlan3dUrl && (
                    <TransformWrapper
            ...
            {activeTab === 'galeria' && unit.galleryImages && unit.galleryImages.length > 0 && (
              ...
              >
                ...
                {/* ── Thumbnails carousel ── */}
                <div className="relative flex-shrink-0 bg-slate-900/95 backdrop-blur-md border-t border-white/10">
                  <div className="flex items-center">
                    ...
                    <div
                      ref={thumbsRef}
                      className="flex items-center gap-2 px-2 py-3 overflow-x-auto scroll-smooth"
                      ...
                      {unit.galleryImages.map((img, i) => (
                        <button
                          ...
                          className={`relative flex-shrink-0 rounded-lg overflow-hidden transition-all duration-200 border-2 ${
                            i === galleryIndex
                              ? 'border-brand-500 w-32 h-20 opacity-100 shadow-md'
                              : 'border-transparent w-28 h-18 opacity-60 hover:opacity-100 hover:scale-105'
                          }`}
                        >
                          <Image src={img} alt={`Thumbnail ${i + 1}`} fill sizes="150px" className="object-cover" />
                        </button>
                      ))}
                    </div>
        ...
        {activeTab !== 'galeria' && (
          <button
            ...
            className="absolute bottom-5 right-16 z-20 bg-slate-900/90 backdrop-blur-md border border-white/10 rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow"
          >
