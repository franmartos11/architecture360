...
interface PolygonCanvasProps {
  ...
  mode?: DrawMode;
  onPointsChange: (id: string, points: { x: number; y: number }[]) => void;
...
export default function PolygonCanvas({ imageUrl, shapes, activeId, mode = 'point', onPointsChange, onComplete }: PolygonCanvasProps) {
  ...
  return (
    <div className="relative w-full select-none rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
      ...
      <svg
        ...
        onMouseLeave={() => { setDragIndex(null); setCursorPos(null); }}
        style={{ cursor: activeShape ? (mode === 'rectangle' ? 'crosshair' : nearFirstPoint ? 'pointer' : 'crosshair') : 'default' }}
      >
        {shapes.map(shape => {
      ...
      {/* Resetear la forma activa (o cancelar un rectángulo a medio arrastrar) */}
      {activeShape && (activeShape.points.length > 0 || rectStart) && (
        <button
          onClick={handleReset}
          title="Borrar todos los puntos de esta forma y empezar de nuevo"
          className="absolute top-2 right-2 flex items-center gap-1.5 bg-white/95 hover:bg-white text-gray-700 text-xs font-medium px-3 py-1.5 rounded-full shadow-lg border border-gray-200 transition-colors"
        >
