...
const CSV_COLUMNS = ['code', 'totalArea', 'status', 'price', 'currency', 'interiorImageUrl'] as const;
const CSV_TEMPLATE_ROW = ['04', '480', 'available', '', 'USD', ''];

// Editor dedicado a lotes de un loteo — a diferencia de FloorUnitsEditor
// (genérico para todos los tipos, con ~18 campos de vivienda que un lote
// no usa), este habla directo con /api/admin/units pero solo con los
// campos que un lote realmente tiene: código, superficie, estado, precio,
// fotos y si ya está delimitado en el plano (derivado de units.polygon,
// no un campo propio — no se puede "marcar como delimitado" sin dibujar
// la silueta de verdad, eso sigue viviendo en la pantalla de Plano).
export default function LotsEditor({ buildingId, floorId, buildingName }: { buildingId: string; floorId: string; buildingName: string }) {
  const typeConfig = useProjectTypeConfig();
  const { unitLabel, showStatus, showPrice } = typeConfig;
  const unitLabelLower = unitLabel.toLowerCase();
  const uAgree = unitAgreement(typeConfig);
  const losLas = uAgree.el === 'la' ? 'las' : 'los';

  const [units, setUnits] = useState<LotRow[]>([]);
  ...
  function dbShape(updates: Record<string, unknown>): Partial<LotRow> {
    const map: Record<string, keyof LotRow> = {
  ...
  const addLot = async () => {
    ...
    const res = await fetch('/api/admin/units', {
      ...
      body: JSON.stringify({ floorId, type: 'lote', code, totalArea: newM2 === '' ? null : Number(newM2) }),
    });
    if (res.ok) {
  ...
  const duplicateLots = async (ids: string[]) => {
    const targets = units.filter(u => ids.includes(u.id));
    const created: LotRow[] = [];
    for (const u of targets) {
      const res = await fetch('/api/admin/units', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          floorId, type: 'lote', code: `${u.code}-copia`, totalArea: u.total_area, status: u.status,
  ...
  const removeLots = async (ids: string[]) => {
  ...
  const handleDownloadTemplate = () => downloadCsv(`plantilla-${unitLabelLower}s.csv`, [[...CSV_COLUMNS], CSV_TEMPLATE_ROW]);

  const handleExportCsv = () => {
    const rows = units.map(u => [
      u.code, String(u.total_area ?? ''), u.status, u.price != null ? String(u.price) : '', u.currency || 'USD', u.interior_image_url ?? '',
    ]);
    downloadCsv(`${unitLabelLower}s.csv`, [[...CSV_COLUMNS], ...rows]);
  };
  ...
  const handleImportCsv = async (file: File) => {
    ...
    for (const row of dataRows) {
      ...
      const res = existingId
        ...
        : await fetch('/api/admin/units', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...payload, floorId, type: 'lote' }) });
        ...
        {selectedIds.size > 0 && (
          <div className="shrink-0 bg-gray-900 rounded-xl px-4 py-2.5 flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <button type="button" onClick={() => duplicateLots(Array.from(selectedIds))} className="h-7 px-2.5 border border-white/25 rounded-md text-xs font-medium text-white/90 hover:bg-white/10 transition-colors">Duplicar</button>
              <button type="button" onClick={() => removeLots(Array.from(selectedIds))} className="h-7 px-2.5 border border-red-400/50 rounded-md text-xs font-medium text-red-300 hover:bg-red-500/15 transition-colors">Borrar</button>
        ...
        {view === 'table' ? (
          <Card className="flex-1 min-h-0 flex flex-col overflow-hidden">
                ...
                return (
                  ...
                  >
                    ...
                    <span className="shrink-0 flex items-center gap-1">
                      <button type="button" title="Duplicar" onClick={e => { e.stopPropagation(); duplicateLots([u.id]); }} className="w-7 h-7 flex items-center justify-center rounded-md text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition-colors">⧉</button>
                      <button type="button" title="Borrar" onClick={e => { e.stopPropagation(); removeLots([u.id]); }} className="w-7 h-7 flex items-center justify-center rounded-md text-gray-400 hover:bg-red-50 hover:text-red-600 transition-colors">×</button>
                    </span>
                  </div>
                );
              })}
            ...
            <div className="shrink-0 border-t border-gray-100 bg-gray-50/60 px-3.5 py-2 flex items-center gap-2 flex-wrap">
              ...
              />
              <button
                type="button" onClick={addLot} disabled={!newCode.trim()}
      ...
      <div className="w-full xl:w-[400px] shrink-0 xl:h-full">
        <Card className="flex flex-col xl:h-full">
              ...
              <div className={`flex flex-col gap-2 p-3.5 rounded-xl border ${cur.polygon && cur.polygon.length > 0 ? 'bg-brand-50 border-brand-100' : 'bg-amber-50 border-amber-200'}`}>
                ...
                <p className="text-[11px] leading-relaxed text-gray-600">
                  {cur.polygon && cur.polygon.length > 0
                    ? 'La silueta ya está dibujada, así que el lote es clickeable desde el masterplan.'
          ...
          <div className="shrink-0 px-5 py-3 border-t border-gray-100 flex items-center justify-between gap-3">
            <button
              type="button" onClick={() => cur && removeLots([cur.id])} disabled={!cur}
