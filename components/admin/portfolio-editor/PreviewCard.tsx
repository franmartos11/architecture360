...
export default function PreviewCard({ avatarImage, name, headline, location, availability, bio, skills, stats }: PreviewCardProps) {
  ...
  return (
    <div className="rounded-[16px] bg-white border border-[rgba(28,25,23,0.08)] overflow-hidden">
      <div className="px-[16px] py-[13px] flex items-center gap-[8px] border-b border-[rgba(28,25,23,0.06)]">
      ...
      <div className="p-[16px] bg-[#f5f4f0]">
        <div className="rounded-[13px] bg-white border border-[rgba(28,25,23,0.08)] overflow-hidden">
          ...
          <div className="px-[14px] pb-[14px] -mt-[24px]">
            <div className="w-[48px] h-[48px] rounded-full border-[3px] border-white flex items-center justify-center font-semibold text-[17px] text-white/90" style={{ background: 'linear-gradient(135deg,#9aa896,#5c7a58)' }}>
              {avatarImage ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={avatarImage} alt="" className="w-full h-full object-cover rounded-full" />
              ) : initial}
            </div>
            <p className="font-semibold text-[13.5px] text-[#1c1a17] mt-[8px]">{name || 'Tu nombre'}</p>
            <p className="text-[11.5px] leading-[1.45] text-[rgba(28,25,23,0.55)] mt-[2px]">{headline || 'Sumá un titular profesional'}</p>
            {location && <p className="text-[10.5px] text-[rgba(28,25,23,0.42)] mt-[4px]">📍 {location}</p>}
            {availability !== 'busy' && (
              <span className="inline-flex h-[22px] px-[9px] items-center gap-[6px] mt-[8px] rounded-[6px] text-[10.5px] font-medium" style={{ background: 'rgba(92,122,88,0.13)', color: '#3f5a3c' }}>
            ...
            {topSkills.length > 0 && (
              <div className="flex gap-[5px] flex-wrap mt-[10px]">
                {topSkills.map(sk => (
                  <span key={sk.label} className="h-[21px] px-[8px] rounded-[6px] bg-[#f5f4f0] text-[10px] font-medium text-[rgba(28,25,23,0.6)] flex items-center">{sk.label}</span>
                ))}
              </div>
            )}
            <div className="flex gap-[12px] mt-[12px] pt-[11px] border-t border-[rgba(28,25,23,0.07)]">
              {stats.map(st => (
                <div key={st.label}>
