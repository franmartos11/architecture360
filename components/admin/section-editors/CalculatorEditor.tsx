...
export default function CalculatorEditor({ onSaved }: { onSaved?: () => void }) {
  ...
  useEffect(() => {
    fetch('/api/settings')
      .then(res => res.json())
      .then(data => {
        if (data && data.interestRate) setSettings(data);
        setLoading(false);
      })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { toast('Configuración guardada exitosamente.'); onSaved?.(); } else toast('Error al guardar.', 'error');
    } catch {
