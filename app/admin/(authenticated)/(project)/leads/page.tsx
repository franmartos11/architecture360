              ...
              <div key={column.id} className="flex flex-col gap-2.5 rounded-xl p-3" style={{ background: '#f2f4f5', border: '1px solid rgba(16,24,40,.07)', minHeight: 420 }}>
                ...
                <div className="flex-1 overflow-y-auto flex flex-col gap-2.5 pr-0.5">
                  {columnLeads.map(lead => {
                    ...
                    const stale = days > 7 && column.id !== 'cerrado';
                    return (
                    <div key={lead.id} className="bg-white rounded-[10px] p-[12px_13px] flex flex-col gap-[9px]" style={{ border: '1px solid rgba(16,24,40,.09)' }}>
                      <div className="flex justify-between items-start gap-2">
                        <div className="text-[12.5px] font-semibold text-[#101828]">{lead.name ?? 'Sin nombre'}</div>
                      ...
                      <div className="flex items-center gap-1.5 pt-[9px]" style={{ borderTop: '1px solid rgba(16,24,40,.07)' }}>
                        ...
                        <button
                          onClick={() => ci > 0 && updateLeadStatus(lead.id, COLUMNS[ci - 1].id)}
                          disabled={ci === 0}
                        ...
                        <button
                          onClick={() => ci < COLUMNS.length - 1 && updateLeadStatus(lead.id, COLUMNS[ci + 1].id)}
